...
This module is the ONE genetics_mcp_server file outside `sdk/` that the sandbox image
ships, because `sdk/client.py` imports `ToolExecutor` and every SDK function delegates to
it. So what it contains is a boundary decision, not an organisational one: the half of the
executor that orchestrates other services — the sandbox gateway, artifact reads, web and
literature search — lives in `tools/orchestration.py`, which nothing here may name, and
which the image does not contain. Adding an import of it breaks the image build rather
than shipping it.
"""
...
class ToolExecutor:
    ...
    The data-access half. `ServerToolExecutor` in `tools/orchestration.py` subclasses this
    with the rest; server processes construct that one, `sdk/client.py` constructs this.
    """
