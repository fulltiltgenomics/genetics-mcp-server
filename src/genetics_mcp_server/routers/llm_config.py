...
class UserInstructionsResponse(BaseModel):
    """Response containing user's additional instructions."""
...
class UserInstructionsUpdate(BaseModel):
    """Request body for updating user's additional instructions."""

    instructions: str = Field(..., description="Additional instructions to augment the AI")
    comment: Optional[str] = Field(None, description="Optional comment describing the change")


class UserToolDescriptionResponse(BaseModel):
    """Response containing user's tool description override."""
...
class UserToolDescriptionUpdate(BaseModel):
    """Request body for updating user's tool description override."""
...
@router.get(
    "/llm-config/user/instructions",
    summary="Get user's additional instructions",
    response_model=Optional[UserInstructionsResponse],
)
async def get_user_instructions(user: str = Depends(auth_required)):
    """
    Get the current user's additional instructions.
    These are appended to the base system prompt to customize AI behavior.
    """
    if not user:
        raise HTTPException(status_code=401, detail="Authentication required")

    db = get_llm_config_db()
    instructions = db.get_user_instructions(user)
    if instructions is None:
        return None
    return UserInstructionsResponse(
        id=instructions.id,
        instructions=instructions.instructions,
        changed_at=instructions.changed_at.isoformat(),
...
@router.put(
    "/llm-config/user/instructions",
    summary="Update user's additional instructions",
    response_model=UserInstructionsResponse,
)
async def update_user_instructions(
    update: UserInstructionsUpdate,
    user: str = Depends(auth_required),
):
    """
    Save the current user's additional instructions.
    These are appended to the base system prompt to customize AI behavior.
    """
    if not user:
        raise HTTPException(status_code=401, detail="Authentication required")
    ...
    db = get_llm_config_db()
    instructions = db.save_user_instructions(
        user_id=user,
        instructions=update.instructions,
        comment=update.comment,
    )
    logger.info(f"User instructions updated by {user}")
    return UserInstructionsResponse(
        id=instructions.id,
        instructions=instructions.instructions,
        changed_at=instructions.changed_at.isoformat(),
...
@router.delete(
    "/llm-config/user/instructions",
    summary="Delete user's additional instructions",
)
async def delete_user_instructions(user: str = Depends(auth_required)):
    """
    Delete the current user's additional instructions (reset to default).
    """
    if not user:
        raise HTTPException(status_code=401, detail="Authentication required")

    db = get_llm_config_db()
    db.delete_user_instructions(user)
    logger.info(f"User instructions deleted by {user}")
    return {"deleted": True}


@router.get(
    "/llm-config/user/tool-descriptions",
    summary="Get user's tool description overrides",
    response_model=dict[str, UserToolDescriptionResponse],
)
async def get_user_tool_descriptions(user: str = Depends(auth_required)):
    """
    Get the current user's custom tool description overrides.
    Only returns tools that have been customized by this user.
    """
    if not user:
        raise HTTPException(status_code=401, detail="Authentication required")

    db = get_llm_config_db()
    descriptions = db.get_user_tool_descriptions(user)
    return {
        name: UserToolDescriptionResponse(
            id=desc.id,
...
@router.put(
    "/llm-config/user/tool-descriptions/{tool_name}",
    summary="Update user's tool description override",
    response_model=UserToolDescriptionResponse,
)
async def update_user_tool_description(
    tool_name: str,
    update: UserToolDescriptionUpdate,
    user: str = Depends(auth_required),
):
    """
    Save a custom tool description for the current user.
    This overrides the default description for this user only.
    """
    if not user:
        raise HTTPException(status_code=401, detail="Authentication required")
    ...
    db = get_llm_config_db()
    desc = db.save_user_tool_description(
        user_id=user,
        tool_name=tool_name,
        description=update.description,
        comment=update.comment,
    )
    logger.info(f"User tool description '{tool_name}' updated by {user}")
    return UserToolDescriptionResponse(
        id=desc.id,
        tool_name=desc.tool_name,
        description=desc.description,
        changed_at=desc.changed_at.isoformat(),
...
@router.delete(
    "/llm-config/user/tool-descriptions/{tool_name}",
    summary="Delete user's tool description override",
)
async def delete_user_tool_description(
    tool_name: str,
    user: str = Depends(auth_required),
):
    """
    Delete the current user's custom tool description (reset to default).
    """
    if not user:
        raise HTTPException(status_code=401, detail="Authentication required")

    db = get_llm_config_db()
    db.delete_user_tool_description(user, tool_name)
    logger.info(f"User tool description '{tool_name}' deleted by {user}")
    return {"deleted": True}
