...
class UserInstructions:
    """Per-user additional instructions."""
...
@dataclass
class UserToolDescription:
    """Per-user tool description override."""
...
class LLMConfigDB(object, metaclass=Singleton):
    ...
    def get_user_instructions(self, user_id: str) -> UserInstructions | None:
        """Get a user's latest additional instructions."""
        cursor = self._conn.cursor()
        cursor.execute(
            """
            SELECT id, user_id, instructions, changed_at, comment
            FROM user_instructions_history
            WHERE user_id = ?
            ORDER BY changed_at DESC
            LIMIT 1
            """,
    ...
    def save_user_instructions(
        self, user_id: str, instructions: str, comment: str | None = None
    ) -> UserInstructions:
        """Save a new version of a user's additional instructions."""
        cursor = self._conn.cursor()
        cursor.execute(
            """
            INSERT INTO user_instructions_history (user_id, instructions, comment)
            VALUES (?, ?, ?)
            """,
            (user_id, instructions, comment),
        )
        self._conn.commit()
    ...
    def delete_user_instructions(self, user_id: str) -> bool:
        """Delete all of a user's instructions history (soft delete by inserting empty)."""
        # insert an empty instructions entry to represent "deleted"
        cursor = self._conn.cursor()
        cursor.execute(
            """
            INSERT INTO user_instructions_history (user_id, instructions, comment)
    ...
    def get_user_tool_descriptions(
        self, user_id: str
    ) -> dict[str, UserToolDescription]:
        """Get the latest tool description overrides for a user."""
        cursor = self._conn.cursor()
        # get the latest entry for each tool_name
        cursor.execute(
            """
            SELECT h1.id, h1.user_id, h1.tool_name, h1.description, h1.changed_at, h1.comment
            FROM user_tool_descriptions_history h1
            INNER JOIN (
                SELECT tool_name, MAX(changed_at) as max_changed_at
                FROM user_tool_descriptions_history
                WHERE user_id = ?
                GROUP BY tool_name
            ) h2 ON h1.tool_name = h2.tool_name AND h1.changed_at = h2.max_changed_at
            WHERE h1.user_id = ?
            """,
            (user_id, user_id),
        )
        result = {}
        for row in cursor.fetchall():
            # skip entries where description is empty (deleted)
            if row["description"]:
                result[row["tool_name"]] = UserToolDescription(
                    id=row["id"],
                    user_id=row["user_id"],
                    tool_name=row["tool_name"],
                    description=row["description"],
    ...
    def save_user_tool_description(
        self, user_id: str, tool_name: str, description: str, comment: str | None = None
    ) -> UserToolDescription:
        """Save a new version of a user's tool description override."""
        cursor = self._conn.cursor()
        cursor.execute(
            """
            INSERT INTO user_tool_descriptions_history (user_id, tool_name, description, comment)
            VALUES (?, ?, ?, ?)
            """,
            (user_id, tool_name, description, comment),
        )
        self._conn.commit()

        return UserToolDescription(
            id=cursor.lastrowid,
            user_id=user_id,
            tool_name=tool_name,
            description=description,
    ...
    def delete_user_tool_description(self, user_id: str, tool_name: str) -> bool:
        """Delete a user's tool description override (soft delete by inserting empty)."""
        cursor = self._conn.cursor()
        cursor.execute(
            """
            INSERT INTO user_tool_descriptions_history (user_id, tool_name, description, comment)
