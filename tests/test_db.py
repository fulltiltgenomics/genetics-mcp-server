...
class TestLLMConfigDB:
    ...
    def test_save_and_get_user_instructions(self, llm_config_db):
        """Test saving and retrieving user instructions."""
        instructions = llm_config_db.save_user_instructions(
            user_id="user@example.com",
            instructions="Always be helpful",
            comment="Initial setup",
        )

        assert instructions.user_id == "user@example.com"
        assert instructions.instructions == "Always be helpful"
        assert instructions.comment == "Initial setup"

        retrieved = llm_config_db.get_user_instructions("user@example.com")
        assert retrieved.instructions == "Always be helpful"

    def test_get_user_instructions_not_found(self, llm_config_db):
        """Test getting instructions for user without any."""
        instructions = llm_config_db.get_user_instructions("nonexistent@example.com")

        assert instructions is None

    def test_user_instructions_versioning(self, llm_config_db):
        """Test that user instructions are versioned."""
        import time

        llm_config_db.save_user_instructions(
            "user@example.com", "Instructions v1"
        )
        # wait to ensure different timestamp
        time.sleep(1.1)
        llm_config_db.save_user_instructions(
            "user@example.com", "Instructions v2"
        )

        latest = llm_config_db.get_user_instructions("user@example.com")
        assert latest.instructions == "Instructions v2"

    def test_delete_user_instructions(self, llm_config_db):
        """Test deleting user instructions (soft delete)."""
        import time

        llm_config_db.save_user_instructions(
            "user@example.com", "Some instructions"
        )
        # wait to ensure different timestamp for soft delete
        time.sleep(1.1)
        llm_config_db.delete_user_instructions("user@example.com")

        # after deletion, latest should be empty string
        latest = llm_config_db.get_user_instructions("user@example.com")
        assert latest.instructions == ""

    def test_save_and_get_user_tool_descriptions(self, llm_config_db):
        """Test saving and retrieving user tool description overrides."""
        desc = llm_config_db.save_user_tool_description(
            user_id="user@example.com",
            tool_name="search_genes",
            description="My custom description",
            comment="Customized for my workflow",
        )

        assert desc.tool_name == "search_genes"
        assert desc.description == "My custom description"

        descriptions = llm_config_db.get_user_tool_descriptions("user@example.com")
        assert "search_genes" in descriptions
        assert descriptions["search_genes"].description == "My custom description"

    def test_user_tool_descriptions_isolation(self, llm_config_db):
        """Test that user tool descriptions are isolated per user."""
        llm_config_db.save_user_tool_description(
            "user1@example.com", "tool1", "User 1 description"
        )
        llm_config_db.save_user_tool_description(
            "user2@example.com", "tool1", "User 2 description"
        )

        user1_descs = llm_config_db.get_user_tool_descriptions("user1@example.com")
        user2_descs = llm_config_db.get_user_tool_descriptions("user2@example.com")

        assert user1_descs["tool1"].description == "User 1 description"
        assert user2_descs["tool1"].description == "User 2 description"

    def test_delete_user_tool_description(self, llm_config_db):
        """Test deleting a user tool description (soft delete)."""
        import time

        llm_config_db.save_user_tool_description(
            "user@example.com", "search_genes", "Custom description"
        )
        # wait to ensure different timestamp for soft delete
        time.sleep(1.1)
        llm_config_db.delete_user_tool_description("user@example.com", "search_genes")

        descriptions = llm_config_db.get_user_tool_descriptions("user@example.com")
        # deleted tools should not appear in the result
