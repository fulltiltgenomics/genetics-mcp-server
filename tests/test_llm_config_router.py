...
class TestUserInstructionsEndpoints:
    """Tests for user instructions endpoints."""

    def test_get_user_instructions_empty(self, client_with_auth):
        """Test getting instructions when none exist."""
        response = client_with_auth.get("/chat/v1/llm-config/user/instructions")

        assert response.status_code == 200
        assert response.json() is None

    def test_update_user_instructions(self, client_with_auth):
        """Test saving user instructions."""
        response = client_with_auth.put(
            "/chat/v1/llm-config/user/instructions",
            json={
                "instructions": "Always focus on genetics topics.",
        ...
        assert response.status_code == 200
        data = response.json()
        assert data["instructions"] == "Always focus on genetics topics."
        assert data["comment"] == "Initial setup"

    def test_get_user_instructions(self, client_with_auth):
        """Test getting saved instructions."""
    ...
    def test_delete_user_instructions(self, client_with_auth):
        """Test deleting user instructions."""
        import time

        # save instructions first
        client_with_auth.put(
            "/chat/v1/llm-config/user/instructions",
            json={"instructions": "To be deleted"},
        )

        # wait for different timestamp
        time.sleep(1.1)

        response = client_with_auth.delete("/chat/v1/llm-config/user/instructions")

        assert response.status_code == 200
        assert response.json()["deleted"] is True

        # verify instructions are empty after delete
        get_resp = client_with_auth.get("/chat/v1/llm-config/user/instructions")
        assert get_resp.json()["instructions"] == ""


class TestUserToolDescriptionEndpoints:
    """Tests for user tool description override endpoints."""

    def test_get_user_tool_descriptions_empty(self, client_with_auth):
        """Test getting tool overrides when none exist."""
        response = client_with_auth.get("/chat/v1/llm-config/user/tool-descriptions")
    ...
    def test_update_user_tool_description(self, client_with_auth):
        """Test saving a tool description override."""
        response = client_with_auth.put(
            "/chat/v1/llm-config/user/tool-descriptions/search_genes",
            json={
                "description": "My custom description for gene search.",
                "comment": "Customized",
            },
        )
        ...
        assert data["description"] == "My custom description for gene search."

    def test_get_user_tool_descriptions(self, client_with_auth):
        """Test getting saved tool overrides."""
        # save override first
        client_with_auth.put(
            "/chat/v1/llm-config/user/tool-descriptions/search_genes",
            json={"description": "Custom gene search"},
        )
        ...
        assert response.status_code == 200
        data = response.json()
        assert "search_genes" in data
        assert data["search_genes"]["description"] == "Custom gene search"

    def test_update_user_tool_description_empty_rejected(self, client_with_auth):
    ...
    def test_delete_user_tool_description(self, client_with_auth):
        """Test deleting a tool override."""
        import time
        ...
        get_resp = client_with_auth.get("/chat/v1/llm-config/user/tool-descriptions")
        assert "search_genes" not in get_resp.json()
